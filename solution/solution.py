...
class EvalResult:
    ...
    def overall_score(self) -> float:
        ...
        """
        # Ensure numeric values and compute mean
        fa = float(self.faithfulness or 0.0)
        re = float(self.relevance or 0.0)
        co = float(self.completeness or 0.0)
        return (fa + re + co) / 3.0
...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        # Determine relevance per chunk
        relevance_flags: list[int] = []
        for chunk in contexts:
            chunk_tokens = _tokenize(chunk)
            if not chunk_tokens:
                relevance_flags.append(0)
                continue
            overlap = chunk_tokens & expected_tokens
            relevance_frac = len(overlap) / len(expected_tokens)
            relevance_flags.append(1 if relevance_frac >= relevance_threshold else 0)

        num_relevant = sum(relevance_flags)
        if num_relevant == 0:
            return 0.0

        # Compute AP@K (average precision over ranks)
        cum_relevant = 0
        ap_sum = 0.0
    ...
    def run_full_eval(
        ...
        # Determine pass/fail based on answer-side metrics
        passed = (faithfulness >= 0.5) and (relevance >= 0.5) and (completeness >= 0.5)

        # Determine failure type by first-match rules
        failure_type = None
        if not passed:
            if faithfulness < 0.3:
        ...
        # Build EvalResult and optionally compute retrieval metrics
...
class LLMJudge:
    ...
    def __init__(self, judge_llm_fn: Callable[[str], str]) -> None:
        # Store the provided judge callable which accepts a prompt and
        # returns a string response (typically JSON-like).
        if not callable(judge_llm_fn):
            raise ValueError("judge_llm_fn must be a callable that accepts a prompt string")
        self.judge_llm_fn = judge_llm_fn

    def _normalize_score(self, v: Any) -> float:
        try:
            fv = float(v)
        except Exception:
            return 0.5
        # If judge returns a 1-5 score, normalize to 0-1
        if fv > 1.0:
            fv = max(0.0, min(1.0, fv / 5.0))
    ...
    def score_response(
        ...
        """
        # Build a concise prompt for the judge LLM
        prompt_lines = [f"Question: {question}", f"Answer: {answer}", "Rubric:"]
        ...
        try:
            ...
            # If the judge callable fails, return default neutral scores
            scores = {k: 0.5 for k in rubric.keys()}
            return {"scores": scores, "reasoning": f"judge call error: {e}"}

        # Try to parse JSON-like response; fall back to defaults
        import json
        ...
        try:
            ...
            if isinstance(parsed, dict):
                # If parsed contains nested structure, try common shapes
                # Prefer a top-level mapping of criterion->score
                for k in rubric.keys():
                    if k in parsed:
                        scores[k] = self._normalize_score(parsed[k])

                # If no rubric keys found but parsed numeric keys exist, take them
                if not scores:
                    for pk, pv in parsed.items():
                        if isinstance(pv, (int, float)):
                            scores[str(pk)] = self._normalize_score(pv)

                # Extract reasoning if present
                if "reasoning" in parsed and isinstance(parsed["reasoning"], str):
                    reasoning = parsed["reasoning"]
                else:
                    # keep raw as fallback reasoning
                    reasoning = raw
            else:
                # parsed JSON not a dict; fallback
                scores = {k: 0.5 for k in rubric.keys()}
                reasoning = raw
        except Exception:
            # Not JSON — return neutral scores mapped to rubric
            scores = {k: 0.5 for k in rubric.keys()}
            reasoning = raw
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        # Positional bias: first item consistently scores higher than others
        positional_bias = False
        if len(item_avgs) > 1:
            first = item_avgs[0]
            others = sum(item_avgs[1:]) / (len(item_avgs) - 1)
            positional_bias = (first > others + 0.1)
...
class BenchmarkRunner:
    ...
    def run(
        ...
        for pair in qa_pairs:
            ...
            # Forward retrieved_contexts if present, else None
            contexts = pair.retrieved_contexts if getattr(pair, "retrieved_contexts", None) else None
            ...
            # Preserve the original QAPair object on the EvalResult
            res.qa_pair = pair
            results.append(res)
    ...
    def generate_report(self, results: list[EvalResult]) -> dict[str, Any]:
        ...
        def avg(attr: str) -> float:
            if total == 0:
                return 0.0
            return sum(getattr(r, attr, 0.0) or 0.0 for r in results) / total

        # Retrieval averages (only include non-None values)
        ...
        report["failure_types"] = failure_counts
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        base_avg_faithfulness = mean([r.faithfulness for r in baseline_results])
        base_avg_relevance = mean([r.relevance for r in baseline_results])
        base_avg_completeness = mean([r.completeness for r in baseline_results])
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        """
        fa = failure.faithfulness if failure.faithfulness is not None else 0.0
        re = failure.relevance if failure.relevance is not None else 0.0
        co = failure.completeness if failure.completeness is not None else 0.0

        # Identify the lowest-scoring dimension
        scores = {"faithfulness": fa, "relevance": re, "completeness": co}
        sorted_scores = sorted(scores.items(), key=lambda kv: kv[1])
        lowest_name, lowest_val = sorted_scores[0]

        # If multiple equally low scores, signal multiple issues
        if len(sorted_scores) >= 2 and abs(sorted_scores[0][1] - sorted_scores[1][1]) < 1e-8:
            return "Multiple issues detected — review full pipeline"

        if lowest_name == "faithfulness":
            return "Context is missing or irrelevant — improve retrieval"
        if lowest_name == "relevance":
            return "Answer does not address the question — improve prompt clarity"
        if lowest_name == "completeness":
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for i, f in enumerate(failures, start=1):
            ...
            suggestion = suggestions[i - 1] if i - 1 < len(suggestions) else "TBD"
            status = "Open"
            lines.append(f"| {fid} | {ftype} | {root} | {suggestion} | {status} |")
    ...
    def generate_improvement_suggestions(
        ...
        if not failures:
            return [
                "Monitor metrics and set alerts for metric drops",
                "Add end-to-end tests with golden dataset to detect regressions",
                "Establish retriever + generator integration tests",
            ]

        counts = self.categorize_failures(failures)
        suggestions: list[str] = []

        # Prioritize based on common failure types
        if any(k and "halluc" in k.lower() for k in counts.keys()):
            suggestions.append("Implement hallucination checker to filter unsupported claims")
            suggestions.append("Add grounding prompt templates that require citations to context")

        if any(k and "irrelev" in k.lower() for k in counts.keys()):
            suggestions.append("Refine prompt engineering and add explicit question decomposition")

        if any(k and "incomplet" in k.lower() for k in counts.keys()):
            suggestions.append("Increase chunk/window size or add more retrieval candidates to improve coverage")
            suggestions.append("Add few-shot examples demonstrating complete answers to the generator")

        # Generic/system suggestions
        suggestions.append("Improve retriever ranking (use cross-encoder reranker)")
        suggestions.append("Add automated unit tests for answer quality and retrieval metrics")
        ...
        if len(deduped) < 3:
            deduped += ["Perform manual review of failing cases to create targeted fixes"]
